...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     ...
     *transferf(hubblenum,kvalnum,lmax-1)
       ...
       character*150 datdir
       
       datdir = "./"

       call readdata(datdir)
c      do i=1,117
c         print*,i,alm(i)
c      enddo
c       do i=1,10
...
c      print*,xi(7,1,1),xi(7,41,1681)
      ...
      subroutine readdata(datdir)
      character*150 datdir
Cf2py intent(in) datdir
     ...
     *transferf(hubblenum,kvalnum,lmax-1)
      ...
      open(2,file=datdir(1:lnblnk(datdir))//'wmapalm.dat')
      do i=1,dim
...
c      print*,kwav(i),mult(i)
      ...
      do i=1,kvalnum
       ...
       double precision dlogtemp,alogsum,alikelihood
       integer ipiv(117)
...
c                  print*,hubble(i)
                   ...
                   enddo
                 call ratint(hubble,transfer1,hubblenum,h0,tran1,
     *                       dtran1,fail)
                 if (fail) then
                 ...
                 call ratint(hubble,transfer2,hubblenum,h0,tran2,
     *                       dtran2,fail)
                 if (fail) then
...
ccccc  compute -1/2*alm*inv(C_lml'm')*Conjugate(alm) ccccccccccccccccccccccccc
       sumlik=complex(0.d0,0.d0)
       do i=1,117
          do j=1,117
             sumlik=sumlik+almr(j)*clm(j,i)*conjg(almr(i))
          enddo
       enddo
c       print*,sumlik
       alikelihood=exp(-1./2.*dreal(sumlik))
c       print*,flik
       return
       end
