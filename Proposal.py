...
#DONE: modified to allow rotated parameters based on some correlation matrix

#DONE: nb. now a class-factory which specifies the package and unpackage functions.
#      this way, the model just contains the actual class, and not an instance, 
#      which means it is re-inited for each MCMC instance


def GenericGaussianProposal(package=None, unpackage=None):
    
    class GenericGaussianClass(object):
        """
        ...
        def __init__(self, sigmas=None, package=package, unpackage=unpackage,
    ...
    return GenericGaussianClass

### factory function --- NOT WHAT WE NEED???!!!
def makeProposal(proposal=GenericGaussianProposal, package=None, unpackage=None):
    return proposal(package=package, unpackage=unpackage)
